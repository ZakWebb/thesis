...
\todo{get a picture of a TM here}
...
\todo{Give a high level overview of the simulation algorithm?}
...
\end{theorem}
Note that the theorem was proved in the black box model, where the function $f$ was provided via black box.  Assume that $f$ is superlinear in both $n$ and $\log^{5/2}(\tau/\epsilon)$, the time-complexity for simulating such a Hamiltonian is simply the product of the complexity of $f$ with \eq{hamiltonian_query_bound}.  Note that if $f$ is efficient to compute, this is an efficient simulation of the Hamiltonian dynamics.
